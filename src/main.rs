...
use futures_util::{future, pin_mut, stream::SplitSink, SinkExt};
use hyper::body::Buf;
use hyper::{body::HttpBody, client::HttpConnector, Body, Client as HyperClinet, Request};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (cluster, mut events) = ClusterBuilder::new(
        token.clone(),
        Intents::GUILD_BANS | Intents::GUILDS | Intents::GUILD_MESSAGES,
    )
    .build()
    .await?;
                            ...
                            || splitted_newlines.next().unwrap() == "rust"
                        {
                            ...
                            let message = http
                                ...
                                .content(&format!(
                                    ...
                                    response.success,
                                    response.stdout,
                                    response.stderr.replace("`", "\"").to_string()
                                ))
                                .unwrap()
                                .exec()
                                .await;
